...
fixMasternode() {
  ...
  systemctl stop axiv_n$1
  echo "Changing directory"
  cd /var/lib/masternodes/axiv$1
  myDir=$(pwd)
  echo $myDir
  ...
  wget https://github.com/AXIVproject/AXIV/releases/download/v1.6.3/AXIVSnapshot.tgz
  tar zxvf AXIVSnapshot.tgz
  rm AXIVSnapshot.tgz
  chown -R masternode:masternode blocks chainstate sporks zerocoin
  echo "Restarting masternode $1"
  systemctl start axiv_n$1
...
checkStatus() {
  #echo "checkStatus $1"
  statusCommand="/usr/local/bin/axiv-cli -conf=/etc/masternodes/axiv_n$1.conf masternode status 2>&1"
  #echo $statusCommand
...
for index in {1..9}
do
  if [ -e /etc/systemd/system/multi-user.target.wants/axiv_n$index.service ]
  then
